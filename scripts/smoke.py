...
s,b,_=req('POST','/api/rounds',{'newsSearch':{'query':'الذكاء الاصطناعي'}}); check('latest-news round (mock feeds + digest)', s==200 and len(b['posts'])==4 and b['round']['news'] and b['round']['news']['query']=='الذكاء الاصطناعي' and b['round']['news']['headline'].startswith('آخر أخبار') and len(b['round']['news']['sources'])>=3, (b.get('round') or {}).get('news') if s==200 else b)
s,b,_=req('POST','/api/rounds',{'newsSearch':{'query':'x'}}); check('latest-news query too short 400', s==400, b)
s,b,_=req('POST','/api/rounds',{'command':'ابحث وش مستجدات الذكاء الاصطناعي اليوم واكتب لي بوستين'}); check('command round (mock: 2 posts)', s==200 and len(b['posts'])==2 and b['round']['news'] and b['round']['news']['command'] and b['round']['news']['count']==2, (b.get('round') or {}).get('news') if s==200 else b)
...
s,b,_=req('GET','/api/state'); check('state after round', b['stats']['liked']==3 and b['stats']['own']==1 and b['stats']['disliked']==1 and b['stats']['rounds']==4 and b['stats']['profileVersion']>=3, b['stats'])

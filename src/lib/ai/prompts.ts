...
export function buildGenerationUser(input: GenerationUserInput): string {
  ...
  if (input.news) {
    ...
    const n = input.news.count ?? 4;
    const task = input.news.command
      ? `هذا أمر المستخدم حرفياً: "${input.news.command}". نفّذه بالضبط: اكتب ${n} ${n === 1 ? 'بوستاً' : 'بوستات'} كما طلب، والتزم بقيوده إن ذكر قيوداً (موضوع، عدد، لغة، نبرة، طول). المادة أعلاه جُمعت من الويب لتنفيذ أمره؛ إن كانت فيها عدة تطورات فكل بوست عن تطور مختلف (اذكر الجهة والتاريخ حين يهم)، وإن كان الأمر رأياً أو فكرة فاكتب من عندك بأسلوبه. `
      : input.news.query
    ...
    sections.push(
      `=== ${input.news.command ? 'مادة جُمعت لتنفيذ أمر المستخدم' : input.news.query ? 'آخر أخبار يريد المستخدم الكتابة عنها' : 'خبر يريد المستخدم الكتابة عنه'} ===\nالعنوان: ${input.news.headline}\n${input.news.brief}${sources}\n` +
        task +
        (input.news.command ? '' : `الخبر عند البشر يُحكى لا يُلخَّص: الناس على LinkedIn تعيد رواية القصة بتفاصيلها وتصريحاتها ثم تعطي رأيها، لذلك البوست الإخباري أطول من بوستات المستخدم المعتادة: من 5 إلى 10 أسطر، فيها سرد متصل وليست رؤوس أقلام ولا سطراً تلغرافياً لكل فكرة. `) +
  ...
  const total = input.news?.count ?? 4;
  const committed = Math.max(0, total - input.exploratory);
  const task = input.news
    ? `اكتب ${total} ${total === 1 ? 'بوست' : 'بوستات'} LinkedIn ${input.news.command ? 'تنفيذاً للأمر أعلاه' : 'عن الخبر أعلاه'}.\nكل بوست يتناول زاوية مختلفة تماماً.`
    : input.topic
  ...
  sections.push(
    `=== المطلوب (الجولة ${input.roundNumber}) ===\n${task}\n` +
      `التوزيع: ${committed} بوست ملتزم بملف الأسلوب، و${input.exploratory} بوست استكشافي (علّمه exploratory=true). العدد الكلي ${total} لا أكثر.\n` +
      `نوّع الأشكال: لا يتكرر الشكل بين البوستات، ولا نوع الخاتمة، ولا الشكل البصري (عدد الأسطر وطولها).${layout}${shapes}`,
  );
